...
let ord_tp tp =
  ...
  in
  ord_tp <|> (w "Maybe" *> ws *> ord_tp >>| fun t -> OptionParam t)
;;
...
let just_p ptrn = just (ptrn >>| fun p -> [], POption (TSome p), [])
...
let%expect_test "pattern_just_valid" =
  ...
  [%expect
    {|
      ([], (POption (TSome ([], (PConst (OrdinaryPConst (Int 1))), []))), []) |}]
;;
...
let if_then_else e =
  ...
  and+ el_br = word "else" **> e >>= (fun x -> return (TSome x)) <|> return TNone in
...
let list_e e = list_enum e (fun l -> return (EList l, []))
...
let%expect_test "expr_with_Just" =
  ...
  [%expect
    {|
      ((Binop (((FunctionApply ((ESome, []), ((Const (Int 2)), []), [])), []),
          Plus, ((Const (Int 1)), []))),
       []) |}]
;;
...
let%expect_test "expr_with_func_apply_strange_but_valid2" =
  ...
  [%expect
    {|
      ((FunctionApply (((Identificator (Ident "f")), []), (ESome, []),
          [((Const (Int 1)), [])])),
       []) |}]
;;
...
let%expect_test "expr_case_statement" =
  prs_and_prnt_ln (expr Allow_t) show_expr "match x with | 1 -> 1 | _ -> 2";
  [%expect
    {|
      ((Match (((Identificator (Ident "x")), []),
          [(([], (PConst (OrdinaryPConst (Int 1))), []), ((Const (Int 1)), []));
            (([], PWildcard, []), ((Const (Int 2)), []))]
          )),
       []) |}]
;;
...
let%expect_test "expr_tuple" =
  ...
  [%expect
    {|
      ((Tuple (((Identificator (Ident "x")), []), ((Const (Int 1)), []),
          [((Const (Int 2)), []);
            ((Tuple (((Identificator (Ident "x")), []),
                ((Identificator (Ident "y")), []), [])),
             [])
            ]
          )),
...
let%expect_test "expr_tuple_neg" =
  ...
  [%expect
    {|
      ((Tuple (((Neg ((Const (Int 1)), [])), []), ((Const (Int 1)), []), [])), []) |}]
;;
...
let%expect_test "expr_case_neg" =
  prs_and_prnt_ln (expr Allow_t) show_expr "match-1with-1->1";
  [%expect
    {|
      ((Match (((Neg ((Const (Int 1)), [])), []),
          [(([], (PConst (NegativePInt 1)), []), ((Const (Int 1)), []))])),
       []) |}]
;;
...
let%expect_test "fun_binding_simple_strange_but_valid1" =
  prs_and_prnt_ln binding show_binding "let f(x)y = x + y";
  [%expect
    {|
      (Def
